...
class StillFeatureMap(nn.Module):
    ...
    class FeatureMapHH(nn.Module):
        """
        Per-head Householder transform feature map with increasing feature dimension after each reflection.
        """
        def __init__(
            self,
            num_heads: int = 32,
            ...
            self.u = nn.Parameter(torch.randn(num_heads, head_dim, device=device, dtype=torch.float32))

        def _apply_householder(self, x: torch.Tensor) -> torch.Tensor:
            u = F.normalize(self.u, p=2, dim=-1, eps=self.eps).to(x.dtype)

            # Apply all reflections at once without looping
            proj = torch.einsum("bhld, hd -> bhl", x, u)

            x = x - 2.0 * proj.unsqueeze(-1) * u.unsqueeze(-2).unsqueeze(0)

            return x

        def forward(self, x: torch.Tensor) -> torch.Tensor:
            y = self._apply_householder(x)
            return x + y if self.skip_connection else y
    ...
    class SoftmaxDim(nn.Module):
        ...
        def forward(self, x: torch.Tensor, y: torch.Tensor):
            return torch.cat([
                torch.softmax(x, dim=-1), torch.softmax(y, dim=-1)
    ...
    def __init__(self, head_dim: int, num_heads: int):
        ...
        self.hh = self.FeatureMapHH(head_dim=head_dim, num_heads=num_heads)
        self.activation = self.SoftmaxDim(eps=self.eps)

    def forward(self, x):
        # Compute norm of input
        r = torch.linalg.norm(x, dim=-1, keepdim=True)  # [B, H, L, 1]       
        x = F.normalize(self.mlp(self.hh(x)), p=2, dim=-1, eps=self.eps) * r  # Normalize to get unit vectors (h, r, d)
...
class LinearAttention(nn.Module):
    def __init__(
        ...
        elif feature_map == 'still_hedgehog':
            if tie_feature_map_qk:
                self.feature_map_q = self.feature_map_k = StillFeatureMap(head_dim=self.head_qk_dim)
            else:
                self.feature_map_q = StillFeatureMap(head_dim=self.head_qk_dim, num_heads=self.num_heads)
        ...
        self.g_proj.weight.data.fill_(1.0)
    ...
    def _process_qkv(self, hidden_states, attention_mask=None, position_ids=None, past_key_value=None):
        ...
        g = g.view(b, l, *self.q_shape).transpose(1, 2)
...
def pad_prefill(q, k, v, g, f_q, f_k, chunk_size=64):
    B, H, L, D = q.shape
    T = f_q.shape[-1]
    device = q.device
    ...
    if pad_len == 0:
        return q, k, v, g, f_q, f_k, 0

    zeros_kv = torch.zeros(B, H, pad_len, D, device=device, dtype=q.dtype)
    zeros_f  = torch.zeros(B, H, pad_len, T, device=device, dtype=f_q.dtype)

    q_pad  = torch.cat([q, zeros_kv], dim=2)   # [B,H,L+pad,D]
    g_pad  = torch.cat([g, zeros_kv], dim=2)   # [B,H,L+pad,D]
    k_pad  = torch.cat([k, zeros_kv], dim=2)
    v_pad  = torch.cat([v, zeros_kv], dim=2)
    f_q_pad = torch.cat([f_q, zeros_f], dim=2) # [B,H,L+pad,T]
    f_k_pad = torch.cat([f_k, zeros_f], dim=2)

    return q_pad, k_pad, v_pad, g_pad, f_q_pad, f_k_pad, pad_len


def still_attention(Q, K, V, g, F_Q, F_K, chunk_size=64, topk_ratio=0.5):
    """
    Hybrid causal SWA + kl_div-based top-k attention with linear long-tail support.
    """
    B, H, L, D = Q.shape
    T = F_K.shape[-1]
    device = Q.device
    l = chunk_size
    assert L % l == 0, "L must be divisible by chunk_size"
    c = L // l
    topk = l // 2 if topk_ratio == 0.5 else int(l * topk_ratio)

    # ---- 0. chunk reshape ----
    q_c = Q.reshape(B, H, c, l, D)
    k_c = K.reshape(B, H, c, l, D)
    v_c = V.reshape(B, H, c, l, D)
    f_q_c = F_Q.reshape(B, H, c, l, T)
    F_K_c = F_K.reshape(B, H, c, l, T)
    g = g.reshape(B, H, c, l, D)

    # ---- 1. causal attention (same chunk) ----
    scores_lower = torch.einsum("bhcld,bhckd->bhclk", q_c, k_c) / math.sqrt(D)
    scores1 = scores_lower
    idx = torch.arange(l, device=device)
    mask_upper1 = idx.unsqueeze(0) > idx.unsqueeze(1)
    scores_lower1 = scores_lower.masked_fill(mask_upper1.unsqueeze(0).unsqueeze(0).unsqueeze(2), 0.0)

    mask_upper2 = idx.unsqueeze(0) >= idx.unsqueeze(1)
    scores_lower2 = scores_lower.masked_fill(mask_upper2.unsqueeze(0).unsqueeze(0).unsqueeze(2), 0.0)
    mask_upper_eq = idx.unsqueeze(0) == idx.unsqueeze(1)
    scores_lower2 = scores_lower2.masked_fill(mask_upper_eq.unsqueeze(0).unsqueeze(0).unsqueeze(2), float("-inf"))

    # ---- 2. upper triangular attention (from previous chunk) ----
    k_c_shift = torch.zeros_like(k_c)
    k_c_shift[:, :, 1:, :, :] = k_c[:, :, :-1, :, :]
    v_c_shift = torch.zeros_like(v_c)
    v_c_shift[:, :, 1:, :, :] = v_c[:, :, :-1, :, :]
    scores_upper = torch.einsum("bhcld,bhckd->bhclk", q_c, k_c_shift) / math.sqrt(D)
    scores2 = scores_upper.clone()
    mask_lower = idx.unsqueeze(0) <= idx.unsqueeze(1)
    scores_upper = scores_upper.masked_fill(mask_lower.unsqueeze(0).unsqueeze(0).unsqueeze(2), 0.0)
    scores_upper[:, :, 0, :, :] = scores_upper[:, :, 0, :, :].masked_fill(mask_upper1.unsqueeze(0).unsqueeze(0).unsqueeze(2), float("-inf"))

    # ---- 3. merge sliding window scores ----
    scores_window1 = scores_lower1 + scores_upper  # [B,H,c,l,l]
    scores_window1 = scores_window1.reshape(B, H, L, l)
    attn1 = torch.softmax(scores_window1, dim=-1)       # [B,H,L,window_size]

    scores_window2 = scores_lower2 + scores_upper  # [B,H,c,l,l]
    scores_window2 = scores_window2.reshape(B, H, L, l)
    attn2 = torch.softmax(scores_window2, dim=-1)       # [B,H,L,window_size]

    # ---- 4. Reshape by chunk and calculate kl_div ----
    attn_chunk1 = attn1.reshape(B, H, c, l, l)
    attn_chunk2 = attn2.reshape(B, H, c, l, l)

    diag_value = 1.0 / l

    epsilon = 1e-8
    attn_chunk1_smooth = (attn_chunk1 + epsilon) / (1 + l * epsilon)
    attn_chunk2_smooth = (attn_chunk2 + epsilon) / (1 + l * epsilon + diag_value)

    attn_chunk2_smooth[:, :, :, :, -1] = diag_value + epsilon

    kl_div = attn_chunk1_smooth * torch.log(attn_chunk1_smooth / (attn_chunk2_smooth + epsilon))
    kl_div = kl_div.sum(dim=-1)  # [B,H,c,l]

    last_kl_div = kl_div[:, :, -2:, :]                              # [B,H,c,l]

    # ---- 5. top-k selection ----
    _, topk_idx = torch.topk(kl_div, k=topk, dim=-1)  # [B,H,c,topk]

    # ---- 6. top-k KV cache ----
    topk_idx_exp = topk_idx.unsqueeze(-1).expand(-1, -1, -1, -1, D)
    K_s_cache = torch.gather(k_c, dim=3, index=topk_idx_exp)
    K_s_cache = K_s_cache[:, :, :-2, :, :]
    V_s_cache = torch.gather(v_c, dim=3, index=topk_idx_exp)
    V_s_cache = V_s_cache[:, :, :-2, :, :]

    # ---- 7. The remaining part of the long-tail ----
    mask_total = torch.zeros(B, H, c, l, device=device, dtype=torch.bool)
    mask_total.scatter_(dim=3, index=topk_idx, value=True)
    mask_inv = ~mask_total
    K_l = F_K_c[mask_inv].reshape(B, H, c, l - topk, T)
    V_l = v_c[mask_inv].reshape(B, H, c, l - topk, D)

    # ---- 8. Q vs Ks_full (top-k cache) ----
    if c > 2:
        K_s = K_s_cache.reshape(B, H, 1, (c-2) * topk, D).repeat(1, 1, c, 1, 1)
        V_s = V_s_cache.reshape(B, H, 1, (c-2) * topk, D).repeat(1, 1, c, 1, 1)
        Ks_max = K_s.shape[3]
        topk_chunk = Ks_max // (c - 2)
        scores3 = torch.einsum("bhcld,bhckd->bhclk", q_c, K_s) / math.sqrt(D)
        idx_k = torch.arange(Ks_max, device=device).unsqueeze(0)
        chunk_idx = torch.arange(c, device=device).unsqueeze(1)
        prefix_limit = (chunk_idx - 1).clamp(min=0) * topk_chunk
        mask_prefix = idx_k >= prefix_limit
        scores3 = scores3.masked_fill(mask_prefix.unsqueeze(0).unsqueeze(0).unsqueeze(3), float("-inf"))

    # ---- 9. Q vs local K (same chunk lower triangular) ----
    scores1 = scores1.masked_fill(mask_upper1.unsqueeze(0).unsqueeze(0).unsqueeze(2), float("-inf"))

    # ---- 10. Concatenate scores and Vs ----
    if c - 2 > 0:
        scores_total = torch.cat([scores3, scores2, scores1], dim=-1)
        Vs_concat = torch.cat([V_s, v_c_shift, v_c], dim=3)
    else:
        scores_total = torch.cat([scores2, scores1], dim=-1)
        Vs_concat = torch.cat([v_c_shift, v_c], dim=3)

    # ---- 11. softmax + weighted sum ----
    scores_total_max = torch.amax(scores_total, dim=-1, keepdim=True)
    scores_total = torch.exp(scores_total - scores_total_max)
    sum_sm = scores_total.sum(dim=-1, keepdim=True)
    out_chunks = torch.einsum("bhclk,bhckd->bhcld", scores_total, Vs_concat)

    # ---- 12. linear attention on long-tail ----
    KV_l_prefix = K_l.permute(0, 1, 2, 4, 3) @ V_l   # [B,H,c,T,D]
    KV_l_prefix_cumsum = torch.cumsum(KV_l_prefix, dim=2) - KV_l_prefix
    zero = torch.zeros(B, H, 1, T, D, device=KV_l_prefix_cumsum.device, dtype=KV_l_prefix_cumsum.dtype)
    KV_l_prefix_cumsum = torch.cat([zero, KV_l_prefix_cumsum[:, :, :-1, :, :]], dim=2)
    out_l = f_q_c @ KV_l_prefix_cumsum

    K_sum = K_l.sum(dim=3)
    Z_prefix = torch.cumsum(K_sum, dim=2) - K_sum
    zero = torch.zeros(B, H, 1, T, device=Z_prefix.device, dtype=Z_prefix.dtype)
    Z_prefix = torch.cat([zero, Z_prefix[:, :, :-1, :]], dim=2)
    sum_ln = (f_q_c * Z_prefix.unsqueeze(3)).sum(dim=-1, keepdim=True)

    # ---- 13. Final output ----
    y_pred = (out_chunks + out_l * g) / (sum_sm + sum_ln + 1e-6)
    y_pred = y_pred.reshape(B, H, L, D)

    return y_pred, KV_l_prefix_cumsum[:, :, -1, :, :], Z_prefix[:, :, -1, :], last_kl_div, K_s_cache, V_s_cache


class StillAttention(LinearAttention):
      def forward(
        self,
        hidden_states: torch.Tensor,
        attention_mask: Optional[torch.Tensor] = None,
        ...
        **kwargs,
        ...
        """
        b, l, _ = hidden_states.size()
        q, k, v, g, kv_seq_len = self._process_qkv(hidden_states, attention_mask, 
        ...
        if output_attentions:   # stage 1: train
            ...
            # Compute causal sliding window kl_div [B,H,c,l] (c*l=L)
            y_pred, _, _, _, _, _ = still_attention(q.float(), k.float(), v.float(), g.float(), f_q.float(), f_k.float(), chunk_size=chunk_size, topk_ratio=topk_ratio)
            y_pred = y_pred.to(dtype=q.dtype)
            ...
            if past_key_value is None:  # Regular training
                ...
                if f_q.shape[2] == 1 and kv_seq_len > 1 and not self.training:  # Generating    stage 3: decode
                    assert use_cache is True
                    f_kv_state, f_k_state, k_cache, v_cache = past_key_value.update_for_decoding(q, k, v, self.layer_idx, self.feature_map_k, dtype=q.dtype)
                    ...
                    y_true = (torch.einsum('bhmn,bhnd->bhmd', a_sm, v_cache.float())
                              + torch.einsum('bhlf,bhfd->bhld', f_q.float(), f_kv_state.float()) * g)
                    sum_ln = torch.einsum(
                        'bhlf,bhf->bhl', f_q.float(), f_k_state.float())[..., None]
                    y_true = (y_true / (sum_sm + sum_ln)).to(q.dtype) 

                else:  # Stateful training  stage2: prefill
                    # Pad to a multiple of chunk_size for easy block implementation
                    key_states, value_states = k, v
                    q, k, v, g, f_q, f_k, pad_len = pad_prefill(q, k, v, g, f_q, f_k, chunk_size=chunk_size)

                    y_true, KV_l_states, Z_states, kl_div, K_s_cache, V_s_cache = still_attention(q.float(), 
                                        k.float(), v.float(), g.float(), f_q.float(), f_k.float(), chunk_size=chunk_size, topk_ratio=topk_ratio)
                    y_true, KV_l_states, Z_states, kl_div, K_s_cache, V_s_cache = y_true.to(q.dtype), KV_l_states.to(q.dtype), Z_states.to(q.dtype), kl_div.to(q.dtype), K_s_cache.to(q.dtype), V_s_cache.to(q.dtype)
                    y_true = y_true[:, :, :l, :]  # Remove the pad part

                    past_key_value.update(key_states, value_states, kl_div, K_s_cache, V_s_cache, pad_len,
                                          KV_l_states, Z_states, self.layer_idx, window_size=self.window_size)
...
class StillDecoderLayer(LlamaDecoderLayer):
    def __init__(self, config: StillConfig, layer_idx: int):
        super().__init__(config, layer_idx) # layer_idx
        self.hidden_size = config.hidden_size

        self.self_attn = StillAttention(config=config, layer_idx=layer_idx)
...
class StillPreTrainedModel(LlamaPreTrainedModel):

    config_class = StillConfig
    base_model_prefix = "model"
    supports_gradient_checkpointing = True
    _no_split_modules = ['StillAttentionDecoderLayer']
...
class StillModel(LlamaModel, StillPreTrainedModel):

    def __init__(self, config: StillConfig):
        ...
        self.layers = nn.ModuleList(
            [StillDecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
...
class StillModelForCausalLM(LlamaForCausalLM, StillPreTrainedModel):
    _tied_weights_keys = ["lm_head.weight"]

    def __init__(self, config):
        super().__init__(config)
        self.model = StillModel(config)
        self.vocab_size = config.vocab_size
...
class LinearAttentionSlidingWindowCache(LinearAttentionState):
    ...
    def __init__(self, window_size: int = 64) -> None:
        ...
        self.pad_len: List[int] = []
        self.last_chunk_kl_div: List[torch.Tensor] = []

        self.k_last_chunk: List[torch.Tensor] = []
        self.v_last_chunk: List[torch.Tensor] = []

        self.k_cache: List[torch.Tensor] = []
        self.v_cache: List[torch.Tensor] = []
        self.window_size = window_size

    def update(self, key_states, value_states, last_chunk_kl_div,
            k_s_cache, v_s_cache, pad_len, 
            KV_l_states, Z_states, layer_idx, window_size=None):

        dtype = key_states.dtype

        while len(self.k_last_chunk) <= layer_idx:
            self.k_last_chunk.append(None); self.v_last_chunk.append(None)
            self.last_chunk_kl_div.append(None); self.pad_len.append(0)
            self.k_cache.append(None); self.v_cache.append(None)
            self.kv_states.append(None); self.k_states.append(None)

        # ---------- 1. save padding + kl_div ----------
        self.pad_len[layer_idx] = pad_len
        self.last_chunk_kl_div[layer_idx] = last_chunk_kl_div.to(dtype)   # [B,H,2,l]

        # ----------2. save full last chunk (no top-k removed) ----------
        window_size = min(2*last_chunk_kl_div.shape[-1], key_states.shape[-2])
        self.k_last_chunk[layer_idx] = key_states[..., -window_size:, :].to(dtype)               # [B,H,2*l,D]
        self.v_last_chunk[layer_idx] = value_states[..., -window_size:, :].to(dtype)             # [B,H,2*l,D]

        # ---------- 3. append top-k chunk cache ----------
        self.k_cache[layer_idx] = k_s_cache.to(dtype)   # [B,H,c-2,topk,D]
        self.v_cache[layer_idx] = v_s_cache.to(dtype)   # [B,H,c-2,topk,D]

        # ---------- 4. update prefix accumulations ----------
        self.kv_states[layer_idx] = KV_l_states.to(dtype)   # [B,H,T,D]
        self.k_states[layer_idx] = Z_states.to(dtype)       # [B,H,T]


    def update_for_decoding(self, querys: torch.Tensor, keys: torch.Tensor, values: torch.Tensor, 
                            layer_idx: int, feature_map_k: Callable, dtype: torch.dtype):
        """
        Update the decoding KV and K states, and KV cache, during decodeing
        """
        with torch.no_grad():
            l = self.last_chunk_kl_div[layer_idx].shape[-1]
            pad = self.pad_len[layer_idx]

            # =============== 1. Process the last chunk: whether to perform top-k ===============
            if pad == 0 and self.last_chunk_kl_div[layer_idx].shape[2] == 2:
                # ---- top-k (lowest kl_div) ----
                ent = self.last_chunk_kl_div[layer_idx][:, :, 0, :]                 # [B,H,l]
                self.last_chunk_kl_div[layer_idx][:, :, 0, :] = self.last_chunk_kl_div[layer_idx][:, :, 1, :]   # shift up
                _, idx = torch.topk(ent, k=self.k_cache[layer_idx].size(-2), dim=-1)
                idx_exp = idx.unsqueeze(-1).expand(-1,-1,-1, self.k_last_chunk[layer_idx].size(-1))

                # ---- split Ks / Kl ----
                K_all = self.k_last_chunk[layer_idx][:, :, :l, :]   # [B,H,l,D]
                V_all = self.v_last_chunk[layer_idx][:, :, :l, :]

                K_s = torch.gather(K_all, dim=-2, index=idx_exp)          # [B,H,topk,D]
                V_s = torch.gather(V_all, dim=-2, index=idx_exp)

                B, H, l = ent.shape
                topk = idx.shape[-1]
                mask = torch.ones(B, H, l, device=K_all.device, dtype=torch.bool)
                mask.scatter_(dim=-1, index=idx, value=False)  # 对每个 (b,h) 各自 scatter
                K_l = K_all[mask].reshape(B, H, l - topk, K_all.size(-1))
                V_l = V_all[mask].reshape(B, H, l - topk, V_all.size(-1))

                # append new top-k to chunk-wise cache
                self.k_cache[layer_idx] = torch.cat([self.k_cache[layer_idx], K_s.unsqueeze(2)], dim=2)
                self.v_cache[layer_idx] = torch.cat([self.v_cache[layer_idx], V_s.unsqueeze(2)], dim=2)

                # ---- accumulate prefix (linear attention) ----
                F_K_l = feature_map_k(K_l)                               # [B,H,l-topk,T]
                self.kv_states[layer_idx] += torch.einsum("bhlt,bhld-> bhtd", F_K_l.float(), V_l.float()).to(dtype)
                self.k_states[layer_idx]  += F_K_l.sum(dim=-2)           # [B,H,T]

                # sliding window shrink size
                self.pad_len[layer_idx] = l - 1
            elif pad == 0 and self.last_chunk_kl_div[layer_idx].shape[2] == 1:
                t = self.last_chunk_kl_div[layer_idx]
                zero_tensor = torch.zeros(t.shape, dtype=t.dtype, device=t.device)
                self.last_chunk_kl_div[layer_idx] = torch.cat([t, zero_tensor], dim=2)
                self.pad_len[layer_idx] = l - 1
            else:
                self.pad_len[layer_idx] -= 1

            # =============== 2. Update sliding window chunk ================
            # left shift and write latest key/value
            if self.k_last_chunk[layer_idx].shape[-2] < 2 * l:
                self.k_last_chunk[layer_idx] = torch.cat([self.k_last_chunk[layer_idx], keys], dim=-2)
                self.v_last_chunk[layer_idx] = torch.cat([self.v_last_chunk[layer_idx], values], dim=-2)
            else:
                self.k_last_chunk[layer_idx] = torch.cat([self.k_last_chunk[layer_idx][..., 1:, :], keys], dim=-2)
                self.v_last_chunk[layer_idx] = torch.cat([self.v_last_chunk[layer_idx][..., 1:, :], values], dim=-2)

            # =============== 3. Update kl_div for sliding window ================
            # attention: q @ kᵀ
            att1 = torch.einsum("bhid,bhjd->bhij", querys.float(), self.k_last_chunk[layer_idx][:, :, -l:, :].float())
            att2 = att1.clone()
            att2[..., -1] = float("-inf")
            ...
            epsilon = 1e-8
            att1 = (att1 + epsilon) / (1 + l * epsilon)
            att2 = (att2 + epsilon) / (1 + l * epsilon + diag_value)

            att2[..., -1] = epsilon + diag_value

            kl_div = att1 * torch.log(att1 / (att2 + epsilon))
            new_ent = kl_div.sum(dim=-1)  # [B,H,1]

            # Write to the last valid position
            pos = l - self.pad_len[layer_idx] - 1
            if self.last_chunk_kl_div[layer_idx].shape[2] == 2:
                self.last_chunk_kl_div[layer_idx][:, :, 1, pos] = new_ent.squeeze(-1)
            else:
                self.last_chunk_kl_div[layer_idx][:, :, 0, pos] = new_ent.squeeze(-1)

            B, H, c, topk, D = self.k_cache[layer_idx].shape
            kc = self.k_cache[layer_idx].reshape(B, H, c * topk, D)
            vc = self.v_cache[layer_idx].reshape(B, H, c * topk, D)
            eff = 2 * l - self.pad_len[layer_idx]
            eff = min(eff, self.k_last_chunk[layer_idx].shape[-2])
            kl_eff = self.k_last_chunk[layer_idx][..., -eff:, :]
            vl_eff = self.v_last_chunk[layer_idx][..., -eff:, :]
            K_out = torch.cat([kc, kl_eff], dim=-2)
            V_out = torch.cat([vc, vl_eff], dim=-2)
        return self.kv_states[layer_idx], self.k_states[layer_idx], K_out, V_out