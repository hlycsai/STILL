        ...
        if self.load_best_model_at_end:  # Return best checkpoint
            try:
                if isinstance(model, PeftModel):
                    adapter_path = self.best_val_checkpoint_path
                    print(f'-> Return best adapter_path: {self.best_val_checkpoint_path}')
                    return adapter_path
                else:
                    model.from_pretrained(self.best_val_checkpoint_path)
                    print(f'-> Loaded best checkpoint from {self.best_val_checkpoint_path}')
