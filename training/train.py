...
def train(config):
    ...
    if config.model.name == "liger_gla":
        ...
        pretrained_config = AutoConfig.from_pretrained(config.model.pretrained_model_name_or_path)
        liger_model_config = StillConfig.from_pretrained(
            config.model.pretrained_model_name_or_path,
            **pretrained_config.to_dict()
        )
        trainer = DefaultTrainer
    elif config.model.name == "still_ar":
        # second stage
        from still.models.still import StillConfig
        pretrained_config = AutoConfig.from_pretrained(config.model.pretrained_model_name_or_path)
        liger_model_config = StillConfig.from_pretrained(
    ...
    for name, param in model.named_parameters():
        ...
        if "train_o" in config.train and config.train.train_o and "self_attn.o_proj" in name:
            param.requires_grad = True
        if "train_g" in config.train and config.train.train_g:
            param.requires_grad = True
    ...
    if  "train_g" in config.train and config.train.train_g and config.train.train_g_lora:
        target_modules.append("self_attn.g_proj")
    # lolcats attention transfer
    if config.model.name == "lolcats_at" or config.model.name == "still_at":
        for name, param in model.named_parameters():
    ...
    if len(target_modules) == 0:
        ...
        adapter_save_path = trainer.save_path + '/best_adapter.txt'
        with open(adapter_save_path, 'w') as f:
            f.write(best_model)
        print(f'\n-> Saved best adapter checkpoint path to: {adapter_save_path}!')
